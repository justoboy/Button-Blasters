...
def GetPlayers(): #Returns a list of player saves
    Players = []
    for name in glob.glob(Directory+'\\Data\\Players\\*.dat'): #Iterates over all .dat files in the Players folder
...
def CreatePlayer(Name): #Creates a new player
    ...
    File = open(Directory+'\\Data\\Players\\{}.dat'.format(Name), 'wb') #Creates a binary file in the players folder with its name the same as the player's
    pickle.dump(Data,File) #Saves the dictionary in the file
    
def LoadPlayer(Name): #Loads a player's data
    File = open(Directory+'\\Data\\Players\\{}.dat'.format(Name), 'rb') #Opens a player save file with the given name
    return pickle.load(File) #Returns the dictionary contained inside

def SavePlayer(Data):
    File = open(Directory+'\\Data\\Players\\{}.dat'.format(Data['Name']), 'wb') #Opens a player save file with the name stored in the given dictionary 
